...
class article {
    ...
    function article($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','level','title','introduce','thumb','proimages','content','tag','author','copyfrom','fromurl','status','hits','addtime','edittime','ip','template','islink','linkurl','filename');
    }
	...
	function set($post) {
		...
		$post['proimages'] = serialize($post['proimages']);
		return $post;
	}
	...
	function get_list($condition = 'status=3', $order = 'addtime DESC', $cache = '') {
		...
		*/
		...
		while($r = $this->db->fetch_array($result)) {
			...
			$r['alt'] = $r['title'];
			$r['title'] = $r['title'];
			$r['proimages'] = unserialize($r['proimages']);
