...
class admin {
	...
	function get_list($condition) {
		...
		*/
		$admins = array();
		$result = $this->db->query("SELECT * FROM {$this->pre}member WHERE $condition ORDER BY userid ASC");// LIMIT $offset,$pagesize");
		while($r = $this->db->fetch_array($result)) {
			$r['lastlogintime'] = timetodate($r['lastlogintime'], 5);
			$r['adminname'] = $CFG['founderid'] == $r['userid'] ? '<span class="f_red">网站创始人</span>' : '<span class="f_blue">超级管理员</span>';
			$admins[] = $r;
		}
		return $admins;
	}
	...
	function get_right($userid) {
		...
		while($r = $this->db->fetch_array($result)) {
			@include RE_ROOT.'/'.($r['moduleid'] == 1 ? 'admin' : 'module/'.$MODULE[$r['moduleid']]['module'].'/admin').'/config.inc.php';
