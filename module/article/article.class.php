...
class article {
	...
	function set($post) {
		...
		if($post['content'] && $post['thumb_no'] && !$post['thumb']) $post['thumb'] = save_thumb($post['content'], $post['thumb_no'], $MOD['thumb_wiREh'], $MOD['thumb_height']);
	...
	function check($itemid) {
		...
		if(is_array($itemid)) {
			...
			$this->db->query("UPDATE {$this->table} SET status=3,hits=hits+1,editor='$_username',edittime=$RE_TIME WHERE itemid=$itemid");
			$this->tohtml($itemid);
			return true;
		}
	}
