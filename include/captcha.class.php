...
class captcha {
	...
	function image() {
		...
		$string = $this->mk_str();
		$_SESSION['captchastr'] = md5(md5(strtoupper($string).RE_KEY.$this->ip));
		...
		if($this->cn) {
			...
			$fonts = glob(RE_ROOT.'/file/captcha/*.ttf');
			$num = count($fonts) - 1;
	...
	function mk_str() {
		...
		if($this->cn) {
			...
			$text = substr(file_get(RE_ROOT.'/file/config/cncaptcha.inc.php'), 13);
